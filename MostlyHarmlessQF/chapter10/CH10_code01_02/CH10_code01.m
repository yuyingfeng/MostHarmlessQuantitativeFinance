...
for mm=1:iter
    % Antithetic Sampling method
        [P_T,N_T]= simul_GBM_antithetic_sampling(S0,r,vol,N_sim,N_time,T);
        Ssim_T =[P_T,N_T];
        ...
        CT_bus  = bullspread_payoff(Ssim_T,K1,K2);% bull spread
        CT_bes  = bearspread_payoff(Ssim_T,K1,K2);% bear spread
        CT_srd   = straddle_payoff(Ssim_T,K);%straddle
        CT_stg   = strangle_payoff(Ssim_T,K1,K2);%strangle
        CT_rvs   = riskreversal_payoff(Ssim_T,K1,K2);%risk reversal
